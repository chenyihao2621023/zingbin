...
<template>
    <div v-transfer-dom>
        <div class="taskMangePopupWrap">
            <popup v-model="showChoseLable" height="100%" width="100%" position="right" :popup-style="{'overflow': 'hidden'}">
                <div class="taskMangeLableWrap">
                    ...
                    <div class="taskManageLableContent-scroll">
                        <div class="warning" v-if="tagCells.length<=0">{{nodata}}</div>
                        <div class="taskLableVariable" v-if="tagCells.length>0">
                            <div class="tasklableContentWrap" v-for="(tag,index) in tagCells" :key="index">
                                <div class="tasklableIcon">
                                    <input type="checkbox" name = "select" :value="index" id="checkbox_a1" class="chk_1">
                                    <label for="checkbox_a1"></label>
                                    <div class="tasklableMine" :style="'background-color:' + tag.tagIndex">{{tag.content}}</div>
                                    <span class="tasklableEdit" @click="showLabelConfirm(index)">编辑</span>
                                </div>
                            </div>
                        </div>
                        <div class="tasklableBottom">
                            <div>
        ...
        <!--标签弹出框-->
        <div class="taskMangeTransfet" v-transfer-dom>
            <confirm v-model="showLabel"
                     ...
                     @on-hide="onHide">
                <slot>
                    <div class="taskMangeTransfetbox">
        ...
        methods: {
            ...
            selectLable() {
                ...
                this.$http.get(url,{ params:Params })
                    .then(res => {
                      setTimeout(() => {
                          this.updatetags(true,tags)
                      },2500)
                    })
                    .catch(err => {
                        console.log(err)
                    });
                    this.$emit("selectLable")
            },
            addLable(){
                this.showLabel = true
            },
                    ...
                    .tasklableContentWrap
                        ...
                        .tasklableIcon
                            ...
                            .chk_1 + label
                                ...
                                height 30px
                                display inline-block
                                background url(choosePng/unchecked.png)
                                background-size 30px
                                background-repeat: no-repeat
                                margin-left -5px
                            .chk_1:checked + label
                                background url(choosePng/checked.png)
                                background-size 30px
                                background-repeat no-repeat
                        .tasklableMine
                            flex 4
