<template>
  <div class="appointAppendVoucher">
    <!-- 头部 -->
    <zing-head :isShowQuery="true" title="指定附表现金流量">
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-input v-model="scope.row.fileId" disabled></el-input><!--@click.native="onfileIdClick(scope.row)"-->
          </template>
        </el-table-column>
        <el-table-column
          prop="attForm_Name"
        ...
        >
          <template slot-scope="scope">
            <el-input v-model="scope.row.attForm_Name" @click.native="onattFormClick(scope.row)" disabled></el-input>
          </template>
        </el-table-column>
        <el-table-column
          prop="pzOriginalAccount"
    ...
    <cast-flow-change :showPopup="showAssist" :recordType="recordType" @backClick="clickBackAssist" @checkItem="WayAssist"></cast-flow-change>
  ...
  export default {
    ...
    methods: {
      ...
      getDataList() {
        //获取当前行所在的现金科目凭证集合
        let pz = this.pzRowId;
        var url = "/zingbiz/report/finance_report/getAppendVoucherItem";
      ...
      },onattFormClick(row) { //附表项目

        this.recordType = 'attachForm';
        this.showAssist = true;
        this.attOtherRow = row;
      },clickBackAssist() {
        this.showAssist = false
      },
      WayAssist(item) {
        this.showAssist = false
        this.$set(this.otherRow , "attForm_Name" , item.fileId)
        this.$set(this.otherRow , "attForm_RowId" , item.rowId)
      },
      saveVoucherInfo() {
        let that = this;
        let flag = true;
