    ...
    <task-selection :showTaskSelection="taskSelectionShow" @confirmTaskSelection="confirmTaskSelection" @cancleTaskSelection="cancleTaskSelection"></task-selection>
        ...
        <div class="contentWrap">
              ...
              <div class="addvariable wrapper-better" ref="wrapper">
                   <div>
                            ...
                            <group label-align="left">
                                ...
                                <x-input class="fontSize" title="名称" placeholder="请输入项目名称"  v-model="taskName">
                                    <span slot="label">名称<span style="color: red">*</span></span>
                                </x-input>
                                <cell title="成员" style="margin-top: 3px" class ="cheng" :class="{'changeH':chengH}" placeholder="">
                                    <span slot="title">成员<span style="color: red">*</span></span>
                                    <div class="iconSize" v-for="cyr in insertNew" :key="cyr" v-if="cyr.value">
                                        <avatar :size="'default'" shape="square" :src='"/ImgRender?imgpath=/checkJpg/"+cyr.value.substring(0,2)+"/"+cyr.value+".jpg&amp;thumb=80x80&amp;errpath=/JsLib/dist/app/images/defaultImgs/userDef_.jpg&amp;compress=true&amp;time=1.13180419_1816&amp;type=.jpg"' style=" float: left;margin-top: 7px;margin-left: 7px;"></avatar>
                                    </div>
                                    <i class="icon iconfont" @click="selectInputPerson()">&#xe64e;</i>
                                </cell>
                                <cell title="负责人" class="fuze" :class="{'fuzeH':fuzeH}" placeholder="">
                                    <span slot="title">负责人<span style="color: red">*</span></span>
                                    <div class="iconSize" v-for="fuze in insertNew2" :key="fuze" v-if="fuze.value">
                                        <avatar :size="'default'" shape="square" :src='"/ImgRender?imgpath=/checkJpg/"+fuze.value.substring(0,2)+"/"+fuze.value+".jpg&amp;thumb=80x80&amp;errpath=/JsLib/dist/app/images/defaultImgs/userDef_.jpg&amp;compress=true&amp;time=1.13180419_1816&amp;type=.jpg"' style=" float: left;margin-top: 7px;margin-left: 7px;"></avatar>
                                    </div>
                                    <i class="icon iconfont" @click="selectInputPerson2()">&#xe64e;</i>
                                </cell>
                                <!-- <cell title="选择分类" class="selectClass" :class="{'classH':classH}" v-model="selectWorkType" placeholder="">
                                    <span slot="title">选择分类</span>
                                    <div class="iconSize2">
                                       {{selectWorkType}}
                                       <i class="icon iconfont iconjia" @click="selectClass()">&#xe64e;</i>
                                    </div>
                                </cell> -->
                                <!-- <cell title="选择类型" class="typeClass" :class="{'typeH':typeH}" v-model="showChoseType" placeholder="">
                                    <span class="iconSize2">
                                        <span v-for="(type,index) in showChoseType">
                                             {{type}}
                                        </span>
                                        <i class="icon iconfont" @click="selectType()">&#xe64e;</i>
                                    </span>
                                </cell> -->
                                <cell title="选择模板" class="mouldClass" :class="{'mouldH':mouldH}" v-model="showChoseMould" placeholder="">
                                    <span class="iconSize2">
                                        <span v-for="(moudle,index) in showChoseMould" :key="index">
                                           {{moudle}}
                                        </span>
                                    </span>
                                    <i class="icon iconfont" @click="selectMould()">&#xe64e;</i>
                                </cell>
                             
                             <!-- <cell title="任务集图标" class="taskIcon" v-model="taskIcon">
                                    <div class="taskIconWrap">
                                        <i :class="['iconfont', `icon-${taskIcon}`]" @click="selectIocn()" :style="{ 'background-color': taskIconBgColor}"></i>
                                    </div>
                                </cell> -->
                                
                                <cell title="项目图标" class="taskIcon" v-model="taskIcon">  
                                  <div class="taskIconWrap">
                                        <i :class="['iconfont', `icon-${taskIcon}`, taskIconBgColor]" @click="selectIocn()" style="background: rgb(112, 204, 109);"></i>
                                  </div>        
                                </cell>
                                 
                                <cell title="将此项目添加到模板" class="mould" @click.native="selectMouldThis()" placeholder="">
                                    <check-icon :value.sync="checked" type="plain"></check-icon>
                                </cell>
                                
                                <cell v-show="projectIsCreated" title="添加子任务" class="typeClass" :class="{'typeH':typeH}"  placeholder="">
                                    <span class="iconSize2">
                                         
                                        <i class="icon iconfont" @click="taskSelectionShow = true">&#xe64e;</i>
                                    </span>
                                </cell>
            ...
            <div class="bottomWrap">
                ...
                <flexbox style="   ">
                            <flexbox-item style="margin-left: 0px;float: left;width: 50%;" >
                                <x-button @click.native="closeEditModel" style="background-color: #40affc;color:#fff">取消
                                </x-button>
                            </flexbox-item>
                            <flexbox-item style="margin-left: 0px;float: left;width: 50%;">
                                <x-button @click.native="addOrEdit()"
                                          style="background-color: #ff8000;color:#fff;"> 保存
                                </x-button>
                            </flexbox-item>
                </flexbox>
                <!-- <div>
                    <x-button v-if="!projectIsCreated" style="height:50px" class="subject-picker-button" type="primary" @click.native="addOrEdit()">提交</x-button>
                    <x-button v-else style="height:50px" class="subject-picker-button" type="primary" @click.native="closeEditModel">关闭</x-button>
                </div> -->
            </div>
        </div>
    </div>
  ...
  methods: {
    ...
    addOrEdit() {
      ...
      if (this.$route.query.taskName) {
        ...
        if(self.taskIcon === 'tianjia1') params.templateIcon = 'benbanzushengchandingdanguanli'
        ...
        self.$http
          ...
          .then(res => {
            ...
            this.$router.push("/projectManage/collectiveManage/addChildTask?boardId="+this.boardId);
            // this.$router.push("/projectManage");
          })
          .catch(err => {
            console.log(err);
