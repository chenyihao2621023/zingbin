<template>
  <div class="otherInOrOutVoucherDetail">
    ...
    <div class="main">
      ...
      <group class="voucherItems" style="margin-top:20px;">
        <div class="header" v-show="voucher.pzStatus !=='WTJ' && voucher.pzStatus !== 'DSH' && voucher.storageMethod === 'batch'">
          <div class="header-left">
            {{voucher.voucherType === "otherIn" ? '子入库单' : '子出库单'}}
          </div>
          <div class="header-right" @click="toInOrOutChildVoucher">
        ...
        <cell v-for="(item,index) in voucher.childVoucherInfo" class="cell-blue" :key="index" title="XSTHD20170810000001-01" :link="'/resourceLibrary/goodsWarehouse/otherInOrOutVoucher?pzRowId='+item.rowId+'&parentId='+voucher.rowId" is-link>
          <tag slot="default" :type="item.pzStatus === 'YSH' ? 'green' : item.pzStatus === 'DSH' ? 'danger' : 'warning'">
            {{item.pzStatus === 'YSH' ? '已审核' : item.pzStatus === 'DSH' ? '未审核' : '未提交'}}
          </tag>
        </cell>
      </group>
    </div>
...
export default {
  name: "otherInOrOutVoucherDetail",
  components: {
    ZingHead,
    Tag,
    XButton,
  ...
  methods: {
    ...
    toInOrOutChildVoucher() {
      this.$router.push({
        path: "/resourceLibrary/goodsWarehouse/otherInOrOutVoucher",
        query: {
          voucherType: this.voucher.voucherType,
          parentId: this.voucher.rowId
        }
      });
    },
    reAudit() {
