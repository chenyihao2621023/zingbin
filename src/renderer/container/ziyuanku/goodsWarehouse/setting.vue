<template>
    <div id="cabinet" class="cabinet">
        ...
        <div class="content" :style="{ height: screenHeight + 'px'}">
            ...
            <checklist class="contentTitle" title="所有出/入库单是否必须关联任务" :options="['是', '否']" v-model="withTrello"
                       :max="1">
            </checklist>
            <!--</group>-->
        </div>
        <x-button type="primary" action-type="button" @click.native="submitSetting()">提交</x-button>
    </div>
</template>
