    ...
    <div>
        <ul class="tree-menu">
            <li v-for="(item, index) in data" :key="index">
                <check-icon v-show="deleteItem" type="plain" :value.sync="item.check" @click.native="checkIcon(item)"></check-icon>
                <x-icon v-show="newItem" type="ios-plus-outline" size="24" @click="addItem(item)" class="circle"></x-icon>
                <span @click="editItem(item)">{{item.code}} {{ item[name] || item.name }} </span>
                <i v-if="item.flag" class="iconfont icon-iconfontjiantou5" @click="toggle(item, index)" :class="{ down:!scope[index],up: scope[index]}" ></i>
                <tree v-if="scope[index]"
                      :data="item.children"
                      :newItem="newItem"
