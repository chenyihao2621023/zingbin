<template>
    <div class="workflowMain">
        ...
        <grid v-show="queryParams.wfType!='KH' && queryParams.wfType!='MODULE_TYPE_ZYK' && queryParams.wfType!='MT_GDZC'">
            <grid-item @on-item-click="menuClick('SP')">
                <div class="weui-grid__icon">
                    <i style="color:#ff8000;" class="iconfont icon-danjuchaxun"></i>
                </div>
        ...
        <div v-for="(value,key,index) in tplData">
            ...
            <grid :cols="cols" v-for="(itemData,index) in value" :key="index">
                <grid-item v-for="(tpldata,index) in itemData" :key="index" @on-item-click="onItemClick(tpldata)">
                    <div class="weui-grid__icon">
                        <i v-if="tpldata.templateIcon!=undefined && tpldata.templateIcon.indexOf('zingIcon')>=0" style="color: #fff;background: #ff8000;padding: 5px;border-radius: 5px;margin-left: -6px;" :class="['iconfont', `icon-shenpi1`]"></i>
                        <i v-else="" style="color: #fff;background: #ff8000;padding: 5px;border-radius: 5px;margin-left: -6px;" :class="['iconfont', `icon-${tpldata.templateIcon}`]"></i>
                    </div>
...
//                        self.tplData = retData;
            ...
            onItemClick (tplData) {
                ...
                }else if(templateModuleType=="MT_GDZC"){
                    queryParams = {};
                    queryParams.companyId=tplData.companyId==undefined?"":tplData.companyId;
                    queryParams.tplId =tplId;
                    queryParams.cType=templateModuleType;
            ...
            checkForward(queryParams){
                ...
                if(self.queryParams.wfType=="MODULE_TYPE_ZYK" || self.queryParams.wfType=="MT_GDZC"){
                    ...
                    self.$http.post(url,params)
                        ...
                        }else if(self.queryParams.voucherType=="DBD" || self.queryParams.voucherType=="childOtherIn" || self.queryParams.voucherType=="childOtherOut"){
                            self.$router.push({
                                path:"customLaunchWorkflow",
                                query:queryParams
                            });
