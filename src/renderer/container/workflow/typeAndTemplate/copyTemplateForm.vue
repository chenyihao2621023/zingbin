        ...
        <popup v-model="showPopupTemplateForm"
            ...
            <div class="popupDate-picker-popup">
                ...
                <div class="templateForm">
                    <selector style="font-size: 16px !important;" title="模板类型: " :options="templateTypeArray" placeholder="请选择模板类型"
                              v-model="formParams.templateType" required >
                    </selector>
                    <group v-if="formParams.templateType=='KH'">
                        <x-input style="font-size: 16px !important;" title="技能组类型: " v-model="formParams.skillType"
                                 @click.native="selSkillGroup" :disabled="readOnly" placeholder="请选择技能组">
                        </x-input>
                    </group>
                    <x-input style="font-size: 16px !important;" title="审批流模板名称: "
                             placeholder="请输入模板名称"
                             v-model="formParams.templateName" required>
                    </x-input>
                    <x-textarea style="font-size: 16px !important;" title="审批流模板描述: "
                                placeholder="请输入模板描述"
                                v-model="formParams.templateDesc" required>
                    </x-textarea>
                    ...
                    ></x-switch>
                    <cell style="font-size: 16px !important;" title="可见范围："
                          is-link="true"
...
<script>
    ...
    export default {
        ...
        components: {
            ...
            XSwitch,
            Group,
            XButton,OrgMemberPicker,TagGroup,
        },
        directives: {
            TransferDom
        },
        ...
        methods: {
            initPageData(){
                ...
                vm.certifChooseData = [];
                if (vm.typeData.moduleType==undefined || vm.typeData.moduleType === "typeForPuTongShenPi" || vm.typeData.moduleType === "") {
            ...
            handleKJFW(){
                let kjfwArr = this.formData.roleTags==undefined || this.formData.roleTags==""?[]:this.formData.roleTags.split(" ");
            ...
            submit() {
                ...
                if (vm.formData.templateId != undefined && vm.formData.templateId!="") {
                    ...
                    url = "/zingbiz/workflow/typeAndTemplate/copyOneTemplate";
                    delete vm.formParams.templateId;
                }
                if (vm.formParams.templateType === undefined || vm.formParams.templateType === "") {
                    vm.$vux.toast.text("请选择模板类型","top");
                    return;
                }
                if (vm.formParams.templateName === undefined || vm.formParams.templateName === "") {
                    vm.$vux.toast.text("请输入模板名称","top");
                    return;
                }
                if (vm.formParams.templateDesc === undefined || vm.formParams.templateDesc === "") {
                    vm.$vux.toast.text("请输入模板描述","top");
                    return;
                }
                if(vm.typeData.templateType === "KH"){
                    if(vm.formParams.skillType==undefined || vm.formParams.skillType==""){
            ...
            getVoucherType(){
                ...
                if(self.templateTypeModel!=undefined && self.templateTypeModel.length!=0){
                    return;
                }
                ...
                self.$http.get(sendUrl,params).then(res => {
                    ...
                    let pzType = self.formParams.templateTypeForOtherModule==undefined?"":self.formParams.templateTypeForOtherModule;
                    if(pzType!=""){
