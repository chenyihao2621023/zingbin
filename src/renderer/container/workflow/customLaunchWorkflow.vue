<template>
    <div class="customLaunchWorkflow">
        <div :style="headClass">
        ...
        <div style="margin-top:51px;">
            ...
            <div style="border-top: 1px solid #ccc;border-bottom: 1px solid #ccc;height:8px;background-color:#EFEFF4;"></div>
            <div style="margin-top: 20px;position: sticky;width: 100%;bottom: 0px;margin-bottom: -14px;">
                <flexbox>
                    <flexbox-item style="margin-left: 0px !important;">
                        <x-button :disabled="isButtonDisabled" style="font-size: 16px;" type="primary" class="btn-save" @click.native="doSubmit('ZC')">暂 存</x-button>
                    </flexbox-item>
                    <flexbox-item style="margin-left: 0px !important;">
                        <x-button :disabled="isButtonDisabled" style="font-size: 16px;" type="primary" class="btn-common" @click.native="doSubmit('tj')">提 交</x-button>
                    </flexbox-item>
                </flexbox>
            </div>
            <br/>
...
<script>
    ...
    import showNodeAndSelExecutor from "@/container/workflow/formTemplate/showNodeAndSelExecutor_v2";
    ...
    export default {
        ...
        methods: {
            handleQueryParams() {
                ...
                this.queryParams.gdzcId =
                    this.$route.query.gdzcId == undefined ||
                    this.$route.query.gdzcId == "undefined"
                        ? ""
                        : this.$route.query.gdzcId;
            ...
            doSubmit(submitType) {
                ...
                if (
                    ...
                    self.queryParams.cType != undefined &&
                    self.queryParams.cType == "MT_GDZC"
                ) {
                    self.createWFForGDZC(submitType);
                } else {
            ...
            handleFormParams() {
                if (
                    ...
                    this.queryParams.cType != undefined &&
                    this.queryParams.cType == "MT_GDZC"
                ) {
                    let subFormParams = this.$refs.pzForm.formParams;
                    this.formParams = this.mergeTowJson(this.formParams, subFormParams);
                } else {
            ...
            vForm(params) {
                let self = this;
                //验证当前模板是否设置了发起节点
                let isSettingFaQiNode = false;
                if(self.nodeDataForTemplate!=undefined && self.nodeDataForTemplate.length>0){
                    self.nodeDataForTemplate.forEach(item =>{
                        if(item.isFaQiNode=="是"){
                            isSettingFaQiNode = true;
                            //获取发起节点id
                            self.faQiNodeId = item.nodeId;
                        }
                    })
                }
                if(!isSettingFaQiNode){
                    self.$vux.toast.text("模板未设置发起节点，不允许发起审批。", "top");
                    return false;
                }
                if (
                    self.queryParams.cType != undefined &&
                    ...
                    self.queryParams.cType != undefined &&
                    (self.queryParams.cType == "MODULE_TYPE_ZYK" || self.queryParams.cType == "MT_GDZC")
                ) {
                    //资源库凭证审批表单验证
                ...
                //验证发起节点子节点的必填项
                let subNodeParams = [];
                if(self.subNodeData!=undefined){
                    subNodeParams = self.subNodeData[self.faQiNodeId]==undefined?[]:self.subNodeData[self.faQiNodeId];
                }
            ...
            createWorkFlow(submitType) {
                ...
                if(self.subNodeData!=undefined){
                    let postSubNodeData = {
                        subNodeData : self.subNodeData[self.faQiNodeId]
                    };
            ...
            createWFForZYK(submitType) {
                ...
                if (submitType != undefined && submitType == "ZC") {
                    ...
                    self.$http
                        ...
                        let isJumpToZyk = true;
                        if(voucherType=="DBD" || voucherType=="childOtherIn" || voucherType=="childOtherOut"){
                            isJumpToZyk = false;
                        }
                        ...
                        let woowId = res.data.data.pzWorkId;
                        if(isJumpToZyk){
                            self.$router.push({
            ...
            createWFForGDZC(submitType) {
                ...
                let url = "/zingbiz/fixedAssets/assetsManagement/updateStatus";
                if (!Z_IsEmpty20(self.queryParams.id)) {
                    params.workflow_id = self.queryParams.id;
                }
                params.cardId = self.queryParams.gdzcId;
                params.orderId = self.queryParams.gdzcId;
                params.cardStatus = "DSH";
                params.moduleType = "MT_GDZC";
                //处理发起节点中的子节点数据
                if(self.subNodeData!=undefined){
                    let postSubNodeData = {
                        subNodeData : self.subNodeData[self.faQiNodeId]
                ...
                if (submitType != undefined && submitType == "ZC") {
                    ...
                    self.$http
                        ...
                        let woowId = res.data.data.workId;
                        setTimeout(function(){
                            self.$router.push({
                                path: "showNode",
                                query: {
            ...
            getAllPageData() {
                ...
                //子节点数据
                self.subNodeData = Z_IsEmpty20(allData.data.subNodeData)?{}:allData.data.subNodeData;
                //发起节点是否与暂存数据
                let faQiSubNodeData = Z_IsEmpty20(allData.data.faQiSubNodeData)?[]:allData.data.faQiSubNodeData;
                let faQiNodeId = Z_IsEmpty20(allData.data.faQiNodeId)?[]:allData.data.faQiNodeId;
                if(faQiSubNodeData!=undefined && faQiSubNodeData.length>0){
                    for(let i in self.subNodeData){
                        if(i==faQiNodeId){
                            self.subNodeData[i] = faQiSubNodeData;
                        }
                    }
                }
            })
